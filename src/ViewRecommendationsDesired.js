...
export default function ViewRecommendationsDesired() {
  const {DesPID} = useParams();
  const [jobs, setJobs] = useState()

  useEffect(() => {
    axios.get(`http://127.0.0.1:5000/get_recommendations_profile/${DesPID}`).then((data) => setJobs(data.data));
  }, [DesPID]);
